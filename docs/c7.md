...
1. What kind of document is used to describe widely used
Application layer protocols?
a) DHCP
b) RFC
c) APPDOC
...
1. Which of the following is *not* something that the Application layer
worries about?
...
1. What is the command that a web browser sends to a web server
...
1. What is the purpose of the "Content-type:" header when you
...
1. What common UNIX command can be used to send simple commands to 
a web server?
a) ftp
b) ping
c) traceroute
...
1.  What happens to a server application that is sending a large
